...
gogFinalField = pd.read_csv('builded_or_modified_cat/gogFinalField.csv')
...
delta_Veps      = np.arange(1,10.5,0.5)               # range of Veps
...
Xage = massPhoto.isochroneSelector(data           = gogFinalField,
                                   age            = AGE,
                                   deltaMag       = [1e6,0.5],
                                   dataMagName    = 'MG',
...
for nind in np.arange(NminStart,len(delta_Nmin),1):
    ...
    for vind in np.arange(0,len(delta_Veps),1):
        ...
        clustersList    = Parallel(n_jobs=Nprocessors)(delayed(NdMethod)\
                                                      (Nmin,
                                                       ...
                                                       Xage,
                                                       sample_class,
                                                       alphaMu) for reps in delta_Reps)
    ...
    with open(f'dataframes_output/Ndmethod/field_characterized_5D_{AGE}.pickle', 'wb') as handle:
