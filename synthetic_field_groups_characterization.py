...
def runDetectionAlgorithm(clusterField,
    ...
    if sample_class == '5d':
        columns1                = ['pmra','pmdec']
        columnsAlgorithm        = ['Nmin','mueps','reps']
...
sample_class    = '6d'                      # this variable specifies if we are working in the 5D or the 6D sample
if sample_class == '6d':
    sample      = 'RVs'
...
if sample_class == '5d':
    errorNames                  = allErrors.drop(['radial_velocity_error','phot_g_mean_mag'],axis=1).columns
...
finalParameterSet   = {}
for nymgInd in np.arange(0,1000+statsForSyntheticNymgs.shape[0],1):
    nymgStats       = statsForSyntheticNymgs.iloc[0]
